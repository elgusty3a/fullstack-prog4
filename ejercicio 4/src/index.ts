...
console.log("\n--- Polimorfismo: procesando pagos ---\n");

const metodosDePago: Procesable[] = [tarjeta, transferencia, billetera];

metodosDePago.forEach((metodo) => {
  console.log(`\n--- Procesando desde ${metodo.constructor.name} ---`);
  console.log(metodo.obtenerDetalles());
  metodo.procesarPago(1000);
});
